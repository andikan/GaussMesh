...
{
	/*
	// Find the neighbor that form triangle from triCandidate
	list<NeighborLit>::iterator it, linkIt[2];
	Triangle** pLink[2];
	int linkNum = 0;
	for(it = this->triCandidate.begin(); it != this->triCandidate.end(); it++)
	{
		if((*it)->e == triangle->edge[0] || (*it)->e == triangle->edge[1] || (*it)->e == triangle->edge[2])
		{
			// Rotate the edge of candidate to (0, 1, 0) such that we only need
			// to compare the orientation of the triangle with 0 and 0.5
			float theta = orientation(this->p, triangle->midPoint);
			theta = (theta < (*it)->theta) ? theta - (*it)->theta + 1 : theta - (*it)->theta;
			// Add to left handside neighbor
			if(theta <= 0.5 && (*it)->tR == NULL)
				pLink[linkNum] = &((*it)->tR);
			// Add to right handside neighbor
			else if(theta > 0.5 && (*it)->tL == NULL)
				pLink[linkNum] = &((*it)->tL);
			// If it should be assigned to a assigned pointer, it shouldn't be assigned
			else
				return false;
			// We have to deal with ticket and tL,tR after we scan all the candidate
			linkIt[linkNum] = it;
			linkNum++;
		}
	}
	// Remove the candidate with ticket == 2
	for(int i = 0; i < linkNum; i++)
	{
		*(pLink[i]) = triangle;
		(*linkIt[i])->ticket++;
		if((*linkIt[i])->ticket == 2)
			this->triCandidate.erase(linkIt[i]);
	}
	return true;*/
