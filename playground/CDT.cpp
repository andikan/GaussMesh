...
void CDT(PointSet& ps)
{
	if(ps.pSet.size() == 2 || ps.pSet.size() == 3)
	{
		...
		mesh.shader->render(GL_ARRAY_BUFFER, mesh.VBid, mesh.assm, 0, mesh.pNum, true);
		glfwSwapBuffers();
		system("pause");
		#endif
		return;
	...
	Point *pbL = *(ps1.pSet.begin());
	Point *pbR = *(ps2.pSet.begin());
...
void bubbleUp(PointSet& ps, vector<Point*>& spine, int upsideDown)
{
	vector<pair<Point*, Point*> > spineSet;
	int baseID = spine[0]->id;
	// Store the spine points with their top points
	for(int i = 0; i < spine.size(); i++)
	{
		float height = avgNeighborLength(spine[i]);
		Point *top = new Point(ps.pSet.size(), spine[i]->p[0], spine[i]->p[1], upsideDown * height);
		spineSet.push_back(pair<Point*, Point*>(spine[i], top));
		ps.addPoint(spineSet[i].second);
	}
	// Link the top and smooth them
	linkSmoothSpine(spineSet);
	#ifdef STEPBYSTEPBUBBLE
	mesh.loadEdgeFromPointSet(*(mesh.ps));
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	mesh.shader->render(GL_ARRAY_BUFFER, mesh.VBid, mesh.assm, 0, mesh.pNum, true);
	glfwSwapBuffers();
	system("pause");
	#endif
	// Find star neighbor and build its curve
	// remind the base may "be" change after finding next ouside neighbor
	Point* base = spineSet[0].first;
	Point* start = base;
	NeighborLit first = findNextOutsideNeighbor(base, base->neighbor.begin(), baseID);
	vector<Point*> curve1 = circumPoint(ps, base, spineSet[base->id-baseID].second, first->p);
	// Find second neighbor
	NeighborLit second = findNextOutsideNeighbor(base, first, baseID);
	vector<Point*> curve2, curvep;			// It will be build in the loop
	// Prevent from recreate the first curve;
	vector<Point*> fCurve = curve1;
	// For each next curve
	while(second->e->p1 != first->e->p1 || second->e->p2 != first->e->p2)
	{
		// Build the second curve
		curve2 = circumPoint(ps, base, spineSet[base->id-baseID].second, second->p);
		if(curvep.size() != 0)
		{
			int num = min<uint>(min<uint>(curvep.size(), curve1.size()), curve2.size());
			for(int i = 1; i < num-1; i++)
			{
				float d1 = curve1[i]->p[2] - curvep[i]->p[2];
				float d2 = curve2[i]->p[2] - curvep[i]->p[2];
				curve1[i]->p[2] = (curve2[i]->p[2] + curvep[i]->p[2]) / 2.0f;
			}
		}
		// Sew two curve
		sew2Curve(curve1, curve2);
		// Find next neighbor
		second = findNextOutsideNeighbor(base, second, baseID);
...
int pruneAndSpine(PointSet &ps)
{
	...
	for(it = ps.triSet.begin(); it != ps.triSet.end(); it++)
	{
		if((*it)->type == TRITYPE::T){		// start from T-triangle
			...
			Point *endPoint = new Point(ps.pSet.size(), 0, 0);
			float radius;
			Edge *currentEdge;
			Triangle* delTri = (*it);
			for(int i = 0; i < 3; i++)
				...
				if(delTri->type == J){					// stop condition, advanced to junction triangle
					...
					for(i = 0; i < spine.size(); i++)
						if(endPoint->p[0] == spine[i]->p[0] && endPoint->p[1] == spine[i]->p[1])
							break;
					Point *preEndPoint = new Point(endPoint->id, endPoint->p[0], endPoint->p[1]);
					for(int j = 0; j < 3; j++)
					...
					if(i != spine.size()){	// visit J triangle twice
						...
						for(int i = 0; i < 3; i++)
							if(delTri->edge[i]->type == Internal && delTri->edge[i] != currentEdge){
								Point* tmp = new Point(ps.pSet.size(), 0, 0);
								for(int j = 0; j < 3; j++)
	...
	for(it = ps.triSet.begin(); it != ps.triSet.end(); it++)
	{
		if((*it)->type == TRITYPE::S){
			Point* tmp[2];
			tmp[0] = new Point(ps.pSet.size(), 0, 0);
			tmp[1] = new Point(ps.pSet.size(), 0, 0);
			int num = 0;
			for(int i = 0; i < 3; i++)
				if((*it)->edge[i]->type == Internal){
			...
			if(i == visitedJTriangleList.size()){
				...
				tmp_mid.push_back(new Point(ps.pSet.size(), 0, 0));
				tmp_mid.push_back(new Point(ps.pSet.size(), 0, 0));
				tmp_mid.push_back(new Point(ps.pSet.size(), 0, 0));
				int j;
				for(int a = 0; a < 3; a++){
